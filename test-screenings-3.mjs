...
async function test() {
  const { data, error } = await supabase.from('screenings').select('*').neq('patientId', 'BGY-041-00217').order('timestamp', { ascending: false }).limit(5);
  console.log('Error:', error);
  console.log('Other screenings from DB:', JSON.stringify(data, null, 2));
}
test();
